...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    @wavelink.WavelinkMixin.listener("on track stuck")
    ...
    async def connnect_command(self, ctx, *, channel: t.Optional[discord.VoiceChannel]):
    ...
    async def next_command_error(self, ctx, exc):
        ...
        elif isinstance(exc, NoMoreItems):
    ...
    async def shuffle_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("The queue could not be shiffled as it is currently empty.")
    ...
    async def repeat_command(self, ctx, mode: str):
        ...
        await ctx.send(f"The repeat mode been set to {mode}.")
    ...
    async def play_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("No music to play as the queue is empty.")
        elif isinstance(exc, NoVoiceChannel):
            await ctx.send("No suitable voice channel was provided.")
def setup(bot):
